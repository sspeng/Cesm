...
implicit none
  ...
  type param
    ...
    integer :: nets, nete, n0_qdp, np1_qdp, rkstage, limiter_option
    integer :: qsize, np, nlev
    real(kind=real_kind), pointer :: qdp_ptr1(:,:,:,:,:)
    real(kind=real_kind), pointer :: qdp_ptr2(:,:,:,:,:)
  end type param
  ...
  param_s%qdp_ptr1 => elem(1)%state%Qdp(:,:,:,:,:)
  param_s%qdp_ptr2 => elem(2)%state%Qdp(:,:,:,:,:)

  print *, param_s%qdp_ptr1(1,1,1,1,1)
