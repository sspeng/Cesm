...
void edgevunpackadvp1_parallel_(edgeAdvp1_param_t *global_param) {
  edgeAdvp1_param_t sl_param;

  int *sl_iwesn, *sl_getmap;
  double *sl_elem_v1, *sl_elem_v; 
  ...
  ldm_alloc_pointer(sl_elem_v1, sl_param.np*sl_param.np*sizeof(FTYPE));  
  ...
  for (ie=_ROW; ie<(sl_param.nete-sl_param.nets+1); ie+=8) {
	...
	pe_get(sl_param.addrs[2]+ie*sl_param.elem_size, sl_elem_v1, sl_param.np*sl_param.np*sizeof(FTYPE));
      if ( q != sl_param.qsize) {
	  pe_get(sl_param.addrs[0]+ie*sl_param.state_size+q*sl_param.nlev*sl_param.np*sl_param.np
	...
	dma_syn();
	  ...
	  //EAST
          sl_elem_v[k*sl_param.np*sl_param.np+i*sl_param.np+sl_param.np-1] =  sl_elem_v[k*sl_param.np*sl_param.np+i*sl_param.np+sl_param.np-1]+sl_receive_east[k*sl_param.np+i];
	  //SOUTH
          sl_elem_v[k*sl_param.np*sl_param.np+i] = sl_elem_v[k*sl_param.np*sl_param.np+i] + sl_receive_south[k*sl_param.np+i];
	  //NORTH
          sl_elem_v[k*sl_param.np*sl_param.np+(sl_param.np-1)*sl_param.np+i] = sl_elem_v[k*sl_param.np*sl_param.np+(sl_param.np-1)*sl_param.np+i] + sl_receive_north[k*sl_param.np+i];
	  //WEST
	    sl_elem_v[k*sl_param.np*sl_param.np+i*sl_param.np] = sl_elem_v[k*sl_param.np*sl_param.np+i*sl_param.np] + sl_receive_west[k*sl_param.np+i];
	  }
      }
    ...
    if ( q != sl_param.qsize){
      pe_put(sl_param.addrs[0]+ie*sl_param.nlev*sl_param.np*sl_param.np+q*sl_param.nlev*sl_param.np*sl_param.np
        , sl_elem_v, sl_param.nlev*sl_param.np*sl_param.np*sizeof(FTYPE));
    }else{
      pe_put(sl_param.addrs[1]+ie*sl_param.nlev*sl_param.np*sl_param.np
        , sl_elem_v, sl_param.nlev*sl_param.np*sl_param.np*sizeof(FTYPE));
    }
    dma_syn();
    }
