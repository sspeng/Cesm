...
//	double qmin[UC*NLEV];
	//double *qmax, *qmin;
  double *Qdp, *dp, *divdp_proj, *dp_l, *Qtens_biharmonic_l, *qmax_val, *qmin_val    \
      , *qmax, *qmin;
  ldm_alloc_pointer(Qdp, block*sizeof(double));
  ldm_alloc_pointer(dp, NLEV*NP*NP*sizeof(double));
  ldm_alloc_pointer(divdp_proj, NLEV*NP*NP*sizeof(double));
  ldm_alloc_pointer(dp_l, NLEV*NP*NP*sizeof(double));
  ldm_alloc_pointer(Qtens_biharmonic_l, block*sizeof(double));
  ldm_alloc_pointer(qmax_val, NLEV*sizeof(double));
  ldm_alloc_pointer(qmin_val, NLEV*sizeof(double));
  ldm_alloc_pointer(qmax, UC*NLEV*sizeof(double));
  ldm_alloc_pointer(qmin, UC*NLEV*sizeof(double));
  ...
  ldm_dealloc(qmin);
  ldm_dealloc(qmax);
  ldm_dealloc(qmin_val);
  ldm_dealloc(qmax_val);
  ldm_dealloc(Qtens_biharmonic_l);
  ldm_dealloc(dp_l);
  ldm_dealloc(divdp_proj);
  ldm_dealloc(dp);
  ldm_dealloc(Qdp);
}
