...
! local
integer :: k,kptr,i,j,ie,ic,q,l,m,n
...
#else
   call t_startf('local_biharmonic_1')
   !if tensor hyperviscosity with tensor V is used, then biharmonic operator is (\grad\cdot V\grad) (\grad \cdot \grad)
   !so tensor is only used on second call to laplace_sphere_wk
...
! Original use of qtens on left and right hand sides caused OpenMP errors (AAM)
         ...
         !DIR$ UNROLL(NP)
                 do i=1,np
                    dsdx00 = dsdx00 + deriv%Dvv(i,l  )*lap_p(i,j  )
                    dsdy00 = dsdy00 + deriv%Dvv(i,l  )*lap_p(j  ,i)
                 end do
           ...
           do j=1,np
              do i=1,np
                 grads(i,j,1)=elem(ie)%Dinv(i,j,1,1)*v1(i,j) + elem(ie)%Dinv(i,j,2,1)*v2(i,j)
                 grads(i,j,2)=elem(ie)%Dinv(i,j,1,2)*v1(i,j) + elem(ie)%Dinv(i,j,2,2)*v2(i,j)
              enddo
           enddo
         !!!!!!!!!!!!!!!! end gradient_sphere !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
           var_coef = var_coef1
           if (var_coef) then
              if (hypervis_power/=0 ) then
                 ...
                 grads(:,:,1) = grads(:,:,1)*elem(ie)%variable_hyperviscosity(:,:)
                 grads(:,:,2) = grads(:,:,2)*elem(ie)%variable_hyperviscosity(:,:)
              else if (hypervis_scaling /=0 ) then
                 ! tensor hv, (3)
                 oldgrads=grads
                 do j=1,np
                    do i=1,np
                       grads(i,j,1) = sum(oldgrads(i,j,:)*elem(ie)%tensorVisc(i,j,1,:))
                       grads(i,j,2) = sum(oldgrads(i,j,:)*elem(ie)%tensorVisc(i,j,2,:))
                    end do
                 end do
              else
           ...
           do j=1,np
              do i=1,np
                 vtemp(i,j,1)=(elem(ie)%Dinv(i,j,1,1)*grads(i,j,1) + elem(ie)%Dinv(i,j,1,2)*grads(i,j,2))
                 vtemp(i,j,2)=(elem(ie)%Dinv(i,j,2,1)*grads(i,j,1) + elem(ie)%Dinv(i,j,2,2)*grads(i,j,2))
              enddo
           enddo
           ...
           !DIR$ UNROLL(NP)
                 do j=1,np
                    qtens(m,n,k,q,ie)=qtens(m,n,k,q,ie)-(elem(ie)%spheremp(j,n)*vtemp(j,n,1)*deriv%Dvv(m,j) &
                                     +elem(ie)%spheremp(m,j)*vtemp(m,j,2)*deriv%Dvv(n,j)) &
                                     * rrearth
                 enddo
               enddo
             enddo
