...
void edgevunpack3_parallel_(edge_param_t *global_param) {
  ...
  if ( _MYID == 0 ) {
    ...
    /*
    if (sl_param.iam == 0){
      cpe_printf("SLAVE: %d %d\n", sl_iwesn[2], sl_iwesn[6]);
    }
    */
  }
  athread_syn(ARRAY_SCOPE, 0xFFFF);
  ...
  for ( ie=_ROW; ie<(sl_param.nete-sl_param.nets+1); ie+=8 ) {
    if ( _COL == 0 ) {
      ...
      bcast_get(sl_param.receive+sl_iwesn[ie*4], sl_receive_west, 4*sl_param.nlev*sl_param.np*sizeof(FTYPE));
      bcast_get(sl_param.receive+sl_iwesn[ie*4+1], sl_receive_east, 4*sl_param.nlev*sl_param.np*sizeof(FTYPE));
      bcast_get(sl_param.receive+sl_iwesn[ie*4+2], sl_receive_south, 4*sl_param.nlev*sl_param.np*sizeof(FTYPE));
      bcast_get(sl_param.receive+sl_iwesn[ie*4+3], sl_receive_north, 4*sl_param.nlev*sl_param.np*sizeof(FTYPE));
   ...
   /* 
    //SWEST
	  ...
	  if (sl_getmap[ie*4*sl_param.max_corner_elem+i] != -1) {
	    pe_get(sl_param.receive+sl_getmap[ie*4*sl_param.max_corner_elem+i], sl_receive_west, 4*sl_param.nlev*sizeof(FTYPE));
	    dma_syn();
	    for (k =0; k<sl_param.nlev; k++){
	      sl_elem_v1[k*sl_param.np*sl_param.np] = sl_elem_v1[k*sl_param.np*sl_param.np] + sl_receive_west[k];
	      sl_elem_v3[k*sl_param.np*sl_param.np] = sl_elem_v3[k*sl_param.np*sl_param.np] + sl_receive_west[3*sl_param.nlev+k];
	    }
	    for (k =0; k<2*sl_param.nlev; k++){
	      sl_elem_v2[k*sl_param.np*sl_param.np] = sl_elem_v2[k*sl_param.np*sl_param.np] + sl_receive_west[sl_param.nlev+k];
	    }
	  }
	}
    ...
    //SEAST
	  ...
	  if ( sl_getmap[ie*4*sl_param.max_corner_elem+i] != -1) {
	    ...
	    dma_syn();
	    for (k=0; k<sl_param.nlev; k++) {
	      sl_elem_v1[k*sl_param.np*sl_param.np+sl_param.np-1] = sl_elem_v1[k*sl_param.np*sl_param.np+sl_param.np-1] + sl_receive_west[k];
	      sl_elem_v3[k*sl_param.np*sl_param.np+sl_param.np-1] = sl_elem_v3[k*sl_param.np*sl_param.np+sl_param.np-1] + sl_receive_west[3*sl_param.nlev+k];
	    }
	    for (k=0; k<2*sl_param.nlev; k++) {
	      sl_elem_v2[k*sl_param.np*sl_param.np+sl_param.np-1] = sl_elem_v2[k*sl_param.np*sl_param.np+sl_param.np-1] + sl_receive_west[sl_param.nlev+k];
	    }
	  }
	}
    //NEAST
    for (i=3*sl_param.max_corner_elem; i<4*sl_param.max_corner_elem; i++) {
	  if ( sl_getmap[ie*4*sl_param.max_corner_elem+i] != -1) {
	    ...
	    dma_syn();
	    for (k=0; k<sl_param.nlev; k++) {
	      sl_elem_v1[k*sl_param.np*sl_param.np+(sl_param.np-1)*sl_param.np+sl_param.np-1] = 
		    sl_elem_v1[k*sl_param.np*sl_param.np+(sl_param.np-1)*sl_param.np+sl_param.np-1] + sl_receive_west[k];
	      sl_elem_v3[k*sl_param.np*sl_param.np+(sl_param.np-1)*sl_param.np+sl_param.np-1] =  
		    sl_elem_v3[k*sl_param.np*sl_param.np+(sl_param.np-1)*sl_param.np+sl_param.np-1] + sl_receive_west[3*sl_param.nlev+k];
	    }
	    for (k=0; k<2*sl_param.nlev; k++) {
	      sl_elem_v2[k*sl_param.np*sl_param.np+(sl_param.np-1)*sl_param.np+sl_param.np-1] = 
		    sl_elem_v2[k*sl_param.np*sl_param.np+(sl_param.np-1)*sl_param.np+sl_param.np-1] + sl_receive_west[sl_param.nlev+k];
	    }
	  }
	}
    //NWEST
    for (i=2*sl_param.max_corner_elem; i<3*sl_param.max_corner_elem; i++) {
	  if ( sl_getmap[ie*4*sl_param.max_corner_elem+i] != -1) {
	    ...
	    dma_syn();
	    for (k=0; k<sl_param.nlev; k++) {
	      sl_elem_v1[k*sl_param.np*sl_param.np+(sl_param.np-1)*sl_param.np] = 
		    sl_elem_v1[k*sl_param.np*sl_param.np+(sl_param.np-1)*sl_param.np] + sl_receive_west[k];
	      sl_elem_v3[k*sl_param.np*sl_param.np+(sl_param.np-1)*sl_param.np] =
		    sl_elem_v3[k*sl_param.np*sl_param.np+(sl_param.np-1)*sl_param.np] + sl_receive_west[3*sl_param.nlev+k];
	    }
	    for (k=0; k<2*sl_param.nlev; k++) {
	      sl_elem_v2[k*sl_param.np*sl_param.np+(sl_param.np-1)*sl_param.np] = 
		    sl_elem_v2[k*sl_param.np*sl_param.np+(sl_param.np-1)*sl_param.np] + sl_receive_west[sl_param.nlev+k];
	    }
	  }
      }  
*/    
    ...
    pe_put(sl_param.addrs[0]+ie*sl_param.nlev*sl_param.np*sl_param.np+start_nlev*sl_param.np*sl_param.np
        , sl_elem_v1+start_nlev*sl_param.np*sl_param.np, block_nlev*sl_param.np*sl_param.np*sizeof(FTYPE) );
    pe_put(sl_param.addrs[1]+ie*2*sl_param.nlev*sl_param.np*sl_param.np+start_2nlev*sl_param.np*sl_param.np
        , sl_elem_v2+start_2nlev*sl_param.np*sl_param.np, block_2nlev*sl_param.np*sl_param.np*sizeof(FTYPE) );
    pe_put(sl_param.addrs[2]+ie*sl_param.nlev*sl_param.np*sl_param.np+start_nlev*sl_param.np*sl_param.np
